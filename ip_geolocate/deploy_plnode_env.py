...
os.system("python vxargs.py -a %s%s -o %s -t 10 -P 100 ssh -oBatchMode=yes -i ~/.ssh/planetlab_rsa %s@{}  -oStrictHostKeyChecking=no \"hostname\"" % (plnode_dir, init_node_filename, output_dir, username))
...
os.system("rm -rf %s" % (output_dir))
os.system("python vxargs.py -a %s%s -o %s -t 300 -P 100 ssh -oBatchMode=yes -i ~/.ssh/planetlab_rsa %s@{}  -oStrictHostKeyChecking=no \"sudo yum -y --nogpgcheck install python-setuptools; python -m pip install requests; sudo python -m pip install requests; python -m pip install json; sudo python -m pip install json\"" % (plnode_dir, deploy_node_filename, output_dir, username))
