...
os.system("python vxargs.py -a %s%s -o %s -t 10 -P 100 ssh -oBatchMode=yes -i ~/.ssh/planetlab_rsa %s@{}  -oStrictHostKeyChecking=no \"mkdir -p ~/%s/data/%s/database; mkdir -p ~/%s/data/%s/plnode; mkdir -p ~/%s/git_repository\"" % (plnode_dir, deploy_node_filename, output_dir, username, projname, taskname, projname, taskname, projname))
